...
**Team-nested API shape.** Every resource the SPA shows in a workspace has a cookie-scoped personal path **and** a team-scoped nested path, and they share the handler logic (just a different `ContextRef`): notes, tags (CRUD + color + rename), todos (CRUD), search/reindex, export/db (owner-only). Team routes live in `TeamsApi.cs`, go through `ResolveTeamAsync` (validates membership + Bearer-context match), and reject `role == Readonly` on writes via `TeamRoleExtensions.CanWrite()`. Bearer tokens bound to a team can only hit their own team's paths; a personal-scope token on a team URL is 403 even when the human behind it is a member — the token's context, not the user's membership, is the authoritative scope.
